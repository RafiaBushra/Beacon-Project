  ...
  for (auto &beaconid : beacons)
  {
        ...
        mainprg_.prev_result.first == MainProgram::ResultType::HIERARCHY)
    {
      ...
      if (res_place != prev_result.end())
      {
        ...
        if (prev_result.size() > 1)
        {
          prefix = MainProgram::convert_to_string(res_index + 1) + ". ";
        }
        namecolor = Qt::red;
        beaconborder = Qt::red;
      }
    ...
    for (auto i : items)
    {
      ...
      if (!selection_clear_in_progress)
      {
        ...
        if (ui->pick_beacon->isChecked())
        {
          ...
          if (!beaconstr.empty())
          {
            ui->lineEdit->insert(QString::fromStdString(beaconstr + " "));
          }
        }
        else
        {
          ...
          if (!coordstr.empty())
          {
            ui->lineEdit->insert(QString::fromStdString(coordstr) + " ");
          }
        }
        output_text(output);
        output_text_end();
