...
typedef std::map<BeaconID, info> map_type; // Defining a shorter name for the data structure since it will be used multiple times in the code to define new variables.

std::minstd_rand rand_engine; // Reasonably quick pseudo-random generator

// Generates a random number within the range @start and @end.
...
// Returns a vector containing all the Beacon IDs.
...
// Checks that the new beacon doesn't already exist in the map and then adds it.
bool Datastructures::add_beacon(
    BeaconID id,
...
// Returns the name of the beacon ID given in the parameter.
std::string Datastructures::get_name(BeaconID id)
{
  if (map_.find(id) != map_.end())
    return map_.at(id).name_;
  return NO_NAME;
}

// Returns the xy coordinates of the beacon ID given in the parameter.
Coord Datastructures::get_coordinates(BeaconID id)
{
  if (map_.find(id) != map_.end())
    return map_.at(id).position_;
  return NO_COORD;
}

// Returns the color of the beacon ID given in the parameter.
Color Datastructures::get_color(BeaconID id)
{
  if (map_.find(id) != map_.end())
    return map_.at(id).colour_;
  return NO_COLOR;
}

// Fills alphabetical_map_. alphabetical_map_ contains the names of the beacons as key and their corresponding id as value. Since std::multimap is sorted by  keys, alphabetical_map_ is automatically sorted as it is being filled. After  that, the values (beacon IDs) are inserted into a vector and returned.
std::vector<BeaconID> Datastructures::beacons_alphabetically()
{
  ...
  std::vector<BeaconID> result_vector;
...
// Similar to beacons_alphabetically() but fills another multimap (brightness_map_) by calculating the brightness of each individual beacon with the help of calculate_brightness() function above.
...
// Similar to min_brightness but returns the most bright beacon's ID. Uses std::min_element() along with a lambda function to compare the brightness of two beacons and returns the least bright beacon's ID.
BeaconID Datastructures::max_brightness()
{

  if (!map_.empty())
  {
    map_type::value_type temp_beacon = *std::max_element(
        map_.begin(), map_.end(),
...
// Manually iterates through all the elements in the map to find the beacon(s) with the given name and fills temp_vector with their corresponding IDs and returns the sorted vector.
...
// Checks that the given beacon ID is valid and changes it's name. Returns true if successful.
bool Datastructures::change_beacon_name(BeaconID id, const std::string &newname)
{
  map_type::iterator temp_iter = map_.find(id);
  if (temp_iter == map_.end())
    return false;
  temp_iter->second.name_ = newname;
  return true;
}

// Checks that the given beacon ID is valid and changes it's color. Returns true if successful.
...
// Checks that the given source beacon does not already send a lightbeam to another beacon and then inserts a lightbeam between them.
bool Datastructures::add_lightbeam(BeaconID sourceid, BeaconID targetid)
{
  if (map_.find(sourceid) == map_.end() || map_.find(targetid) == map_.end() || map_.at(sourceid).beamTarget_ != NO_ID)
    return false;
...
// Sorts the beamSources_ vector and returns it. beamSources_ contains all the beacons that directly send lightbeams to the given id.
...
// Returns a vector containing a "path" of lightbeams starting from the given beacon by iterating through all the direct and indirect "children" (beam sources).
...
// Removes the given beacon from the data structure. It also removes all traces of the beacon by removing its ID from all the lightbeam data structures. Returns true if successful.
...
// A helper function that inserts all the the beacons that let out lightbeams but don't recieve any into @children.
void longest_path_helper(
    const map_type &map_,
    BeaconID most_original_beam,
...
// Uses post-order tree traversal method to find all the children and then calculates the length of each path and returns the longest one.
...
// Returns the sum of rgb components of 2 colors.
...
void total_color_helper(const map_type &map_, BeaconID most_original_beam, std::pair<Color, int> &total_colour)
{
  ...
  {
    if (!map_.at(child).beamSources_.empty())
      total_color_helper(map_, child, total_colour);
    total_colour.first = calculate_total_color(map_.at(child).colour_, total_colour.first);
    total_colour.second++;
  }
}

// Returns the total color of the given beacon.
