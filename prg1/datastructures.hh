...
public:
    ...
    // Estimate of performance: O(N)
    // Short rationale for estimate: Uses a single for loop to iterate through all the 
    // elements and insert them into the return vector.
    ...
    // Short rationale for estimate: Uses std::multimap<> insertion method functions
    // which have logarithmic complexity but then iterates through the map to create
    // a vector which has O(N) complexity.
    ...
    // Estimate of performance: O(N)
    // Short rationale for estimate: Uses a single for() loop to empty the beamSources_
    // vector.
